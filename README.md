...
Docker 모드에서는 DeepCode가 `http://localhost:8000`에서 실행됩니다. Compose 설정에는 Codex 로그인 콜백 포트와 `/root/.codex` 영구 볼륨이 포함되어 있습니다. 기본 Compose 포트 publish는 `127.0.0.1`에만 묶여 있어서 같은 PC의 브라우저에서만 접근됩니다. 앱 포트가 충돌하면 `DEEPCODE_HOST_PORT`로 바꿀 수 있습니다. Docker의 Codex OAuth 콜백은 기본적으로 `1457`을 사용합니다. OpenAI에 등록된 redirect URI 제약 때문에 콜백 포트는 `1455` 또는 `1457`만 사용하세요. `nanobot` 포트는 `NANOBOT_HOST_PORT`로 바꿀 수 있습니다. `deepcode_config.json`은 Settings 화면에서 모델 선택을 저장할 수 있도록 쓰기 가능하게 마운트되지만, 파일이 없을 때 Docker가 디렉터리로 자동 생성하지 않도록 보호되어 있습니다.
...
Docker에서 로그인할 때는 Compose가 기본적으로 `127.0.0.1:1457:1457` 포트를 열어 둡니다. `1457`이 이미 사용 중이면 `CODEX_AUTH_CALLBACK_PORT=1455`로 바꿔 실행하세요. 로컬 방화벽이나 보안 프로그램이 해당 포트를 막고 있으면 로그인 콜백이 실패할 수 있습니다.
...
제공 스크립트(`python deepcode.py`, `deepcode_docker/run_docker.sh`, `nanobot/run_nanobot.sh`)는 Docker cache를 활용하되 기본적으로 현재 checkout의 소스를 기준으로 이미지를 빌드 확인한 뒤 실행합니다.
