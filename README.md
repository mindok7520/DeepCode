...
Docker 모드에서는 DeepCode가 `http://localhost:8000`에서 실행됩니다. Compose 설정에는 Codex 로그인 콜백 포트 `1455`, `1457`과 `/root/.codex` 영구 볼륨이 포함되어 있습니다. 기본 Compose 포트 publish는 `127.0.0.1`에만 묶여 있어서 같은 PC의 브라우저에서만 접근됩니다. 포트가 충돌하면 `DEEPCODE_HOST_PORT`, `CODEX_AUTH_CALLBACK_PORT`, `CODEX_AUTH_FALLBACK_CALLBACK_PORT`, `NANOBOT_HOST_PORT` 환경변수로 바꿀 수 있습니다. `deepcode_config.json`은 Settings 화면에서 모델 선택을 저장할 수 있도록 쓰기 가능하게 마운트되지만, 파일이 없을 때 Docker가 디렉터리로 자동 생성하지 않도록 보호되어 있습니다.
...
Docker에서 로그인할 때는 Compose가 기본적으로 `127.0.0.1:1455:1455`, `127.0.0.1:1457:1457` 포트를 열어 둡니다. 로컬 방화벽이나 보안 프로그램이 해당 포트를 막고 있으면 로그인 콜백이 실패할 수 있습니다.
